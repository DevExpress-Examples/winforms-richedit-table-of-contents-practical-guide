...
Namespace RichEditTOCGeneration
    ...
    Public Partial Class Form1
        ...
        Private Sub ApplyStyles()
            SearchForTOCEntries(Sub(ByVal location, ByVal level) Document.GetParagraph(location).Style = Me.GetStyleForLevel(level))
        End Sub

        Private Sub AssignOutlineLevels()
            SearchForTOCEntries(Sub(ByVal location, ByVal level) Document.GetParagraph(location).OutlineLevel = level)
        End Sub

        Private Sub AddTCFields()
            SearchForTOCEntries(Sub(ByVal location, ByVal level) Document.Fields.Add(location, String.Format("TC ""{0}"" \f {1} \l {2}", Document.GetText(Document.GetParagraph(location).Range), "defaultGroup", level)))
        End Sub
        ...
        Private Sub InsertContentHeading()
            Dim range As DocumentRange = Document.InsertText(Document.Range.Start, "Contents" & Global.Microsoft.VisualBasic.Constants.vbCrLf)
            Dim cp As CharacterProperties = Document.BeginUpdateCharacters(range)
            cp.FontSize = 18
